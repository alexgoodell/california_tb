...
df = pd.read_csv("http://34.201.26.231/limcat/hdd/share/larry-summer_best-guess_15-Aug-2017/masters/basecase-master.csv")

# remove calib data
df = df[df.Year > 2016]

df = df[df.Intervention_id == 0]
...
for y in range(2017,2066):
  year_cases = df[(df.State_name == "Life") & (df.Year == y)].groupby("Iteration_num").Risk_of_prog_total.sum().mean() * Run_adjustment
  cum_cases = df[(df.State_name == "Life") & (df.Year <= y)].groupby("Iteration_num").Risk_of_prog_total.sum().mean() * Run_adjustment
  print "{} cases: {} ({} of total)".format(y, year_cases, cum_cases/total_cases)
