...
# df = pd.read_csv("/Volumes/Main external/remotes nov 13/fb-limited-master.csv")
df = pd.read_csv("/ssd/share/dec_30_best_guess/masters/fb-limited-master.csv")
...
all_uninfected_states = [ "Uninfected TB", "Uninfected Testing TST", "Uninfected Testing QFT", "Uninfected Testing TSPOT", "Uninfected Testing TST+QFT", "Uninfected Testing TST+TSPOT" ] + fp_in_treatment_states


results_df = pd.DataFrame(columns=["year", "nativity", "mean", "min", "max"])
for year in [ 2001, 2014, 2030, 2040, 2050]:
  last_cycle = df[df.Year == year].Cycle_id.max()
  qdf = df[(df.Cycle_id == last_cycle) & (df.State_name.isin(all_uninfected_states))].groupby(df.Iteration_num).Population_fb.sum() / df[(df.Cycle_id == last_cycle) & (df.State_name == "Life")].groupby(df.Iteration_num).Population_fb.sum()
  results_df.loc[len(results_df)+1] = [ year, "FB", 1.0 - qdf.mean(),1.0 - qdf.max(),1.0 - qdf.min()]
  print "FB  {}: {} {} {}".format(year, 1.0 - qdf.mean(),1.0 - qdf.max(),1.0 - qdf.min())
  qdf = df[(df.Cycle_id == last_cycle) & (df.State_name.isin(all_uninfected_states))].groupby(df.Iteration_num).Population_us.sum() / df[(df.Cycle_id == last_cycle) & (df.State_name == "Life")].groupby(df.Iteration_num).Population_us.sum()
  results_df.loc[len(results_df)+1] = [ year, "USB", 1.0 - qdf.mean(),1.0 - qdf.max(),1.0 - qdf.min()]
  print "USB {}: {} {} {}".format(year, 1.0 - qdf.mean(),1.0 - qdf.max(),1.0 - qdf.min())
...
#     results_df.loc[(iteration,intervention_name), "qalys_saved_outside_ca_undiscounted"] =  ldf[(ldf.State_name == 'LTBI treated with RTP') & (ldf.Intervention_name == intervention_name)].left_cures.sum() * 0.0798 # see "QALYs saved from individuals outside ca" worksheet

#     results_df.loc[(iteration,intervention_name), "cost_saved_from_cases_averted_outside_ca_undiscounted"] =  ldf[(ldf.State_name == 'LTBI treated with RTP') & (ldf.Intervention_name == intervention_name)].left_cures.sum() * 929.724 # see "QALYs saved from individuals outside ca" worksheet
...
#     results_df.loc[(iteration,intervention_name), "qalys_saved_outside_ca_discounted"] =  ldf[(ldf.State_name == 'LTBI treated with RTP') & (ldf.Intervention_name == intervention_name)].left_cures.sum() * 0.0248  # see "QALYs saved from individuals outside ca" worksheet

#     results_df.loc[(iteration,intervention_name), "cost_saved_from_cases_averted_outside_ca_discounted"] =  ldf[(ldf.State_name == 'LTBI treated with RTP') & (ldf.Intervention_name == intervention_name)].left_cures.sum() * 304.122 # see "QALYs saved from individuals outside ca" worksheet
...
# final_df.T.to_csv("/ssd/share/" + name + "/results/final_df_" + time.strftime("%Y-%m-%d-%Hh%Mm") + ".csv")
# final_raw_df.to_csv("/ssd/share/" + name + "/results/final_raw_df_" + time.strftime("%Y-%m-%d-%Hh%Mm") + ".csv")
