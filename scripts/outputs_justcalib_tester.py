...
Html_dir = "/hdd/share/" + Output_name +"/results/output_html/"
Fig_count = 0
...
# else:
# df = pd.read_csv("/hdd/share/" + Simulation_name + '/masters/basecase-limited-master.csv',  encoding='ascii')
df = pd.read_csv("/hdd/share/larry-summer_best-guess_15-Aug-2017/masters/full-pop-limited-master.csv")

# df = pd.read_csv("go/tmp/cycle_state/blue-beatle/fb/fb_1494581938508285760.csv",  encoding='ascii')
# df = pd.read_csv("/hdd/share/heavenly-charlie_best-guess_13-Aug-2017/masters/fb-limited-master.csv",  encoding='ascii')
df = df[df.Year < 2015]
df = df[df.Intervention_name == "Basecase"]
print "removed unneeded data"
...
if Include_images:
  ...
  for group in groupings:
    ...
    if chain_name == "HIV":
      # There are two HIV states -- this avoids the error of finding the average. Hack but works. 
      rdf["Population"] = rdf["Population"] * Run_adjustment * 2
    else:
...
udf = df[(df.Intervention_id == 0) & (df.State_name.isin(states_of_interest))]

udf['Source'] = "CAPE"
udf = udf.rename(columns={'State_name': 'Group', 'Iteration_num': 'Iteration'})
fdf = udf.copy()
fdf['Nativity'] = "FB"
udf['Nativity'] = "USB"
fdf['CasesAverage'] = fdf["Risk_of_prog_fb"]
udf['CasesAverage'] = fdf["Risk_of_prog_us"]

m_df = fdf.append(udf)
m_df = m_df[['Nativity', 'Year', 'Group', 'CasesAverage', 'Source', 'Iteration']]

m_df["Group"] = m_df["Group"].replace("Life", "Total")
m_df["Group"] = m_df["Group"].replace(["Infected HIV, no ART","Infected HIV, ART"], "HIV")
m_df["Group"] = m_df["Group"].replace("Smoker", "Smokers")
m_df["Group"] = m_df["Group"].replace("Transplant patient", "Transplant")
m_df["Group"] = m_df["Group"].replace("Less than one year", "Imported")
m_df["Group"] = m_df["Group"].replace("Fast latent", "Recent transmission")
...
html_figure("main_calib_figure","large","Main calib figure")
